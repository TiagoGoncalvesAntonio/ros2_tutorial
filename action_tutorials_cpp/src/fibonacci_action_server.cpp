...
public:
  ...
  : Node("fibonacci_action_server", options)
  {
    ...
    this->action_server_ = rclcpp_action::create_server<Fibonacci>(
      this,
      "fibonacci",
      // These are the 3 callback functions
...
private:
    ...
    std::shared_ptr<const Fibonacci::Goal> goal)
  {
    ...
    // Reject requests larger than 46 to avoid overflow in the Fibonacci sequence.
    if (goal->order > 46) {
      RCLCPP_WARN(this->get_logger(), "Rejecting goal because order is too large");
      return rclcpp_action::GoalResponse::REJECT;
    }

    // This implementation accepts and executes the goal.
  ...
  void handle_accepted(const std::shared_ptr<GoalHandleFibonacci> goal_handle)
  {
    ...
    // Since the execution is a long-running operation, we spawn off a thread to do the actual
    // work and return from handle_accepted quickly.
