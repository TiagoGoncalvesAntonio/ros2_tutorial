...
public:
    ...
    turtle_spawned_(false)
  {
    ...
    // We create a TransformListener, Once the listener is created, it starts receiving tf2 transformations over the wire, and buffers them for up to 10 seconds.
  ...
  void on_timer()
  {
    ...
    if (turtle_spawning_service_ready_) {
      if (turtle_spawned_) {
        ...
        // We query the listener for a specific transformation. We call lookup_transform method with following arguments:
        // - target frame: the frame we want to transform to
        // - source frame: the frame we want to transform from
        // - time: the time at which we want the transformation. We can also specify tf
        try {
